...
bool Renderer::init(GLContext& ctx) {
    ...
    if (useGL_) {
        ...
        if (!renderer2D_.init(sw_, sh_, nullptr)) {
            std::fprintf(stderr, "[Renderer] 2D init failed\n");
            return false;
        }
    } else {
        if (!renderer2D_.init(sw_, sh_, ctx.window)) {
            std::fprintf(stderr, "[Renderer] Software 2D init failed\n");
            return false;
        }
    }
    return true;
}
void Renderer::shutdown() { if (useGL_) renderer3D_.shutdown(); renderer2D_.shutdown(); }
void Renderer::resize(int w, int h) { sw_=w; sh_=h; if (useGL_) renderer3D_.resize(w,h); renderer2D_.resize(w,h); }
...
void Renderer::begin2D(){
    ...
    renderer2D_.beginBatch();
    in2D_=true;
}
void Renderer::end2D()  { if(in2D_)  { renderer2D_.endBatch(); in2D_=false; if(useGL_) ctx_->swap(); } }
